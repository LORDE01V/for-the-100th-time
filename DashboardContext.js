...
import { energyModes, defaultWidgetLayout, themePresets, mockAreas } from '../utils/mockData';
...
export const DashboardProvider = ({ children }) => {
  ...
  const [enabledWidgets, setEnabledWidgets] = useState(() => {
    ...
    let widgets = saved ? JSON.parse(saved) : defaultWidgets;
    // Ensure LoadsheddingStatus is present
    if (!widgets.includes('LoadsheddingStatus')) {
      widgets.push('LoadsheddingStatus');
    }
    return widgets;
  });
  ...
  const [selectedArea, setSelectedArea] = useState(() => {
    ...
    const initialArea = savedArea ? JSON.parse(savedArea) : null;
    // If no saved area, default to the first mock area
    if (!initialArea && mockAreas.length > 0) {
      return mockAreas[0];
    }
    return initialArea;
  });
  ...
  useEffect(() => {
    console.log('selectedArea in DashboardContext:', selectedArea);
